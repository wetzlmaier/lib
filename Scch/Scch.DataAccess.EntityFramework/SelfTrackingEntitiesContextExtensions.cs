...
namespace Scch.DataAccess.EntityFramework
{
    public static class SelfTrackingEntitiesContextExtensions
    {
        ...
        public static void ApplyChanges<TEntity>(this ObjectSet<TEntity> objectSet, TEntity entity)
            where TEntity : class, IObjectWithChangeTracker
        {
            ...
            objectSet.Context.ApplyChanges(
                objectSet.EntitySet.EntityContainer.Name + "." + objectSet.EntitySet.Name, entity);
        }
        ...
        /// <param name="entity">The entity serving as the entry point of the object graph that contains changes.</param>
        public static void ApplyChanges<TEntity>(this ObjectContext context, string entitySetName, TEntity entity)
            where TEntity : IObjectWithChangeTracker
        {
            ...
            try
            {
                ...
                foreach (
                    IObjectWithChangeTracker changedEntity in
                        entityIndex.AllEntities.Where(x => x.ChangeTracker.State == ObjectState.Deleted))
                {
                ...
                foreach (IObjectWithChangeTracker changedEntity in entityIndex.AllEntities)
                {
                    ...
                    foreach (NavigationProperty navProp in entityType.NavigationProperties)
                    {
                        ...
                        {
                            ApplyChangesToIndependentAssociation(context, changedEntity,
                                                                 entry, navProp, relatedEnd, allRelationships);
                        }
        ...
        private static void ApplyChangesToIndependentAssociation(ObjectContext context,
                                                                 IObjectWithChangeTracker changedEntity,
                                                                 ObjectStateEntry entry, NavigationProperty navProp,
                                                                 IRelatedEnd relatedEnd,
                                                                 RelationshipSet allRelationships)
        {
            ...
            else
            {
                if (navProp.ToEndMember.RelationshipMultiplicity == RelationshipMultiplicity.Many)
                {
                    ...
                    ObjectList collectionPropertyChanges;
                    if (changeTracker.ObjectsRemovedFromCollectionProperties.TryGetValue(navProp.Name,
                                                                                         out collectionPropertyChanges))
                    {
                    ...
                    if (changeTracker.ObjectsAddedToCollectionProperties.TryGetValue(navProp.Name,
                                                                                     out collectionPropertyChanges))
                    {
        ...
        private static void HandleRelationshipKeys(ObjectContext context, EntityIndex entityIndex,
                                                   RelationshipSet allRelationships, IObjectWithChangeTracker entity)
        {
                ...
                changeTracker.State == ObjectState.Deleted)
            {
                ...
                foreach (var entityReference in EnumerateSaveReferences(relationshipManager))
                {
                    ...
                    AssociationEndMember fromEnd =
                        associationSet.AssociationSetEnds[entityReference.SourceRoleName].
                            CorrespondingAssociationEndMember;
                    AssociationEndMember toEnd =
                        associationSet.AssociationSetEnds[entityReference.TargetRoleName].
                            CorrespondingAssociationEndMember;
                    ...
                    if (changeTracker.OriginalValues != null)
                    {
                        ...
                        {
                            hasOriginalValue = changeTracker.OriginalValues.TryGetValue(navigationProperty.Name,
                                                                                        out originalValue);
                        }
                    ...
                    if (currentKey != null)
                    {
                        ...
                        else
                        {
                            CreateRelationship(context, entityReference, entry.EntityKey, currentKey,
                                               originalKey == null ? EntityState.Unchanged : EntityState.Added);
                        }
                    }
                    else
                    {
                    ...
                    if (originalKey != null)
                    {
                        ...
                        ObjectStateEntry originalEntry;
                        ...
                        {
                            allRelationships.Add(entityReference, entry.Entity, originalEntry.Entity,
                                                 EntityState.Deleted);
                        }
                        else
                        {
                            ...
                            if (originalEntry != null &&
                                ...
                                associationSet.AssociationSetEnds[fromEnd.Name].CorrespondingAssociationEndMember.
                                    RelationshipMultiplicity != RelationshipMultiplicity.Many)
                            {
                                targetReference =
                                    originalEntry.RelationshipManager.GetRelatedEnd(entityReference.RelationshipName,
                                                                                    entityReference.SourceRoleName) as
                                    EntityReference;
        ...
        private static ObjectStateEntry CreateRelationship(ObjectContext context, EntityReference entityReference,
                                                           EntityKey fromKey, EntityKey toKey, EntityState state)
        {
            if (state != EntityState.Detached)
            {
                AssociationSet associationSet = ((AssociationSet) entityReference.RelationshipSet);
                AssociationEndMember fromEnd =
                    associationSet.AssociationSetEnds[entityReference.SourceRoleName].CorrespondingAssociationEndMember;
                AssociationEndMember toEnd =
                    associationSet.AssociationSetEnds[entityReference.TargetRoleName].CorrespondingAssociationEndMember;
                ...
                bool found = context.TryGetObjectStateEntry(fromKey, toKey, associationSet, fromEnd, toEnd,
                                                            out relationshipEntry);
        ...
        private static EntityState DetachRelationship(ObjectContext context, EntityReference entityReference,
                                                      ObjectStateEntry fromEntry, EntityKey toKey)
        {
            ...
            if (toKey != null)
            {
                ...
                ObjectStateEntry currentRelationshipEntry;

                if (context.TryGetObjectStateEntry(fromEntry.EntityKey, toKey, associationSet, fromEnd, toEnd,
                                                   out currentRelationshipEntry))
                {
        ...
        private static string CreateReferenceKeyLookup(string keyMemberName, EntityReference reference,
                                                       NavigationProperty navigationProperty)
        {
            ...
            return String.Format(CultureInfo.InvariantCulture, "Navigate({0}.{1}).{2}",
                                 reference.RelationshipSet.ElementType.FullName, reference.TargetRoleName,
                                 keyMemberName);
        }
                                                      ...
                                                      IDictionary<string, object> values)
        {
            Debug.Assert(
                ...
                reference.RelationshipSet.ElementType == navigationProperty.RelationshipType,
                "the reference and navigationProperty should coorospond");

            EntitySet entitySet =
                ((AssociationSet) reference.RelationshipSet).AssociationSetEnds[reference.TargetRoleName].EntitySet;
        ...
        private static void MoveSavedReferenceKey(EntityReference reference, object entity,
                                                  NavigationProperty navigationProperty,
                                                  ...
                                                  IDictionary<string, object> targetValues)
        {
            Debug.Assert(
                navigationProperty == null ||
                reference.RelationshipSet.ElementType == navigationProperty.RelationshipType,
                " the reference and navigationProperty should correspond");
        ...
        private static IEnumerable<EntityReference> EnumerateSaveReferences(RelationshipManager manager)
        {
            return manager.GetAllRelatedEnds().OfType<EntityReference>()
                .Where(
                    er =>
                    er.RelationshipSet.ElementType.RelationshipEndMembers[er.SourceRoleName].RelationshipMultiplicity !=
        ...
        public static void StoreReferenceKeyValues(this ObjectContext context, IObjectWithChangeTracker entity)
        {
            ...
            {
                NavigationProperty navigationProperty =
                    entityType.NavigationProperties.FirstOrDefault(
                        n => n.RelationshipType == entityReference.RelationshipSet.ElementType &&
        ...
        private static void HandleEntity(ObjectContext context, EntityIndex entityIndex,
                                         RelationshipSet allRelationships, IObjectWithChangeTracker entity)
        {
        ...
        private static void HandleDeletedEntity(ObjectContext context, EntityIndex entityIndex,
                                                RelationshipSet allRelationships, IObjectWithChangeTracker entity)
        {
            HandleRelationshipKeys(context, entityIndex, allRelationships, entity);
            ChangeEntityStateBasedOnObjectState(context, entity);
        ...
        private static void UpdateOriginalValues(ObjectContext context, IObjectWithChangeTracker entity)
        {
            ...
            foreach (EdmProperty property in entityType.Properties)
            {
                ...
                else if (property.TypeUsage.EdmType is ComplexType)
                {
                    ...
                    UpdateOriginalValues((ComplexType) property.TypeUsage.EdmType,
                                         ObjectContext.GetObjectType(entity.GetType()).FullName, property.Name,
                                         entity.ChangeTracker.OriginalValues, complexOriginalValues);
                }
            }
        }

        private static void UpdateOriginalValues(ComplexType complexType, string entityTypeName,
                                                 string propertyPathToType,
                                                 IDictionary<string, object> originalValueSource,
                                                 OriginalValueRecord complexOriginalValueRecord)
        {
            ...
            foreach (EdmProperty property in complexType.Properties)
            {
                ...
                string propertyPath = String.Format(CultureInfo.InvariantCulture, "{0}.{1}", propertyPathToType,
                                                    property.Name);
                if (property.TypeUsage.EdmType is PrimitiveType &&
                    originalValueSource.TryGetValue(propertyPath, out value))
                {
                    ...
                    {
                        Debug.Assert(complexOriginalValueRecord == null,
                                     "we only throw when the value is not null and the recored is null");
                        throw new InvalidOperationException(
                            String.Format(
                                CultureInfo.CurrentCulture,
        ...
        private static void ChangeEntityStateBasedOnObjectState(ObjectContext context, IObjectWithChangeTracker entity)
        {
            switch (entity.ChangeTracker.State)
            {
                case (ObjectState.Added):
                    ...
                    Debug.Assert(context.ObjectStateManager.GetObjectStateEntry(entity).State == EntityState.Added,
                                 "State should have been Added");
                    break;
        ...
        private static EntityType GetCSpaceEntityType(this MetadataWorkspace workspace, object entity)
        {
            ...
            if (entityType == null)
            {
                throw new ArgumentException(String.Format(CultureInfo.CurrentCulture,
                                                          "Unable to find a CSpace type for type {0}", type.FullName));
            }
            return entityType;
        }
        ...
        private static EntityKey GetCurrentEntityKey(
            this EntityReference entityReference, ObjectContext context)
        {
        ...
        {
            NavigationProperty navigationProperty =
                GetNavigationProperty(entry.ObjectStateManager.MetadataWorkspace.GetCSpaceEntityType(entry.Entity),
        ...
        private static NavigationProperty GetNavigationProperty(this EntityType entityType,
                                                                string navigationPropertyIdentity)
        {
            NavigationProperty navigationProperty;
        ...
        {
            EntitySet entitySet =
                ((AssociationSet) relatedEnd.RelationshipSet).AssociationSetEnds[relatedEnd.TargetRoleName].EntitySet;
            ...
            if (null != relatedEnd.RelationshipSet)
            {
                ...
                foreach (
                    ReferentialConstraint constraint in
                        ((AssociationType) relatedEnd.RelationshipSet.ElementType).ReferentialConstraints)
                {
        ...
        private static bool TryGetObjectStateEntry(this ObjectContext context, EntityKey from, EntityKey to,
                                                   AssociationSet associationSet, AssociationEndMember fromEnd,
                                                   AssociationEndMember toEnd, out ObjectStateEntry entry)
        {
            entry = null;
            foreach (
                var relationshipEntry in
                    (from e in
                         context.ObjectStateManager.GetObjectStateEntries(EntityState.Added | EntityState.Unchanged)
        ...
        private sealed class AddHelper
        {
            ...
            public static EntityIndex AddAllEntities(ObjectContext context, string entitySetName,
            ...
            private void HandleStateManagerChange(object sender, CollectionChangeEventArgs args)
            {
                if (args.Action == CollectionChangeAction.Add)
                {
                    ...
                    var navPropNames =
                        _context.MetadataWorkspace.GetCSpaceEntityType(entity).NavigationProperties.Select(n => n.Name);
                    var entityRefOriginalValues =
                        changeTracker.OriginalValues.Where(kvp => navPropNames.Contains(kvp.Key));
                    foreach (KeyValuePair<string, object> originalValueWithName in entityRefOriginalValues)
                    {
                        if (originalValueWithName.Value != null)
                        {
                            _entitiesDuringAdd.Enqueue(new Tuple<ObjectStateEntry, string, IEnumerable<object>>(
                                                           ...
                                                           new[] {originalValueWithName.Value}));
                        }
                    }

                    // Queue removed collection values
                    foreach (
                        KeyValuePair<string, ObjectList> collectionPropertyChangesWithName in
        ...
        private sealed class EntityIndex
        {
            ...
            public void Add(ObjectStateEntry entry, ObjectChangeTracker changeTracker)
            {
                ...
                {
                    finalKey =
                        _context.CreateEntityKey(temporaryKey.EntityContainerName + "." + temporaryKey.EntitySetName,
                                                 entry.Entity);
                }
        ...
        private sealed class RelationshipSet : IEnumerable<RelationshipWrapper>
        {
            ...
            public void Remove(ObjectStateEntry relationshipEntry)
            {
                ...
                DbDataRecord values = relationshipEntry.State == EntityState.Deleted
                                          ? relationshipEntry.OriginalValues
                ...
                object fromEntity =
                    _context.ObjectStateManager.GetObjectStateEntry((EntityKey) values.GetValue(fromOridinal)).Entity;
                int toOridinal = values.GetOrdinal(associationSet.ElementType.AssociationEndMembers[1].Name);
                object toEntity =
                    _context.ObjectStateManager.GetObjectStateEntry((EntityKey) values.GetValue(toOridinal)).Entity;
        ...
        private sealed class RelationshipWrapper : IEquatable<RelationshipWrapper>
        {
                                         ...
                                         EntityState state)
            {
                ...
                Debug.Assert(null != end0, "null end0");
                Debug.Assert(null != end1, "null end1");
